...
from ucasr.wrappers import create_mixed_loaders, get_model
...
def pretrain_mixed_model(args, wandb_sweep=False, sweep_run_name=None):

    args = set_remote_paths(args, mixed_pretrain=True)
    ...
    if not wandb_sweep:
        ...
        # experiment tag
        exp_tag = f'mixed_{args.audio_context}'
    ...
    # getting dataloaders
    tr_loader, va_loader, tr_eval_loader = create_mixed_loaders(args)

    # get model
    model, loss = get_model(args, mode='mixed_pretrain')
    model.to(device)
    ...
    for epoch in range(args.n_epochs):
        ...
        tr_loss = iterate_dataset(model, tr_loader, loss_function=loss, optimizer=optim, device=device,
                                  is_pretrain=True, mixed_pretrain=True)
...
if __name__ == '__main__':
    ...
    configs = load_yaml('config/mixed_config.yaml')
    ...
    pretrain_mixed_model(parser.parse_args())
