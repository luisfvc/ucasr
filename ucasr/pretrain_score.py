...
from ucasr.wrappers import create_score_loaders, get_model
...
def pretrain_score_model(args, wandb_sweep=False, sweep_run_name=None):

    args = set_remote_paths(args, score_pretrain=True)
    ...
    # getting dataloaders
    tr_loader, va_loader, tr_eval_loader = create_score_loaders(args)

    # get model
    model, loss = get_model(args, mode='score_pretrain')
    model.to(device)
    ...
    print('\nPretraining score encoder finished!\n')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--finetune', help='finetune model after pretrain', action='store_true', default=False)
    parser.add_argument('--ft_audio', help='also finetune on audio encoder', action='store_true', default=False)
    ...
    pretrain_score_model(parser.parse_args())
